  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        ...
        const newBook = {
          coverImage: bookResponse.volumeInfo?.imageLinks?.smallThumbnail || bookResponse.volumeInfo?.imageLinks?.thumbnail || "",
        ...
        setBook({
          ...
          author: "",
          description: "Unable to load book information. Please try again later.",
        });
      }
    };
  ...
  const handleSaveBook = async () => {
    ...
    try {
      ...
      if (!bookData) {
        ...
        const { data: newBookData } = await addBook({ variables: { googleId: bookId } });
    ...
    <div className="max-w-4xl mx-auto py-10 px-4">
      <div className="bg-white rounded-lg shadow p-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-primary1 font-semibold mb-4">Author: {book.author}</p>
          <p className="text-gray-700 mb-6 whitespace-pre-wrap">{book.description}</p>
          ...
          {loading ? (
            ...
            <p className="text-gray-600">Please sign in to save this book to your collection.</p>
          )}
            ...
            <div className="mt-8">
              ...
              <div className="space-y-4">
                {data.bookGoogle.reviews.map((review) => (
                  <div key={review._id} className="border-l-4 border-primary1 pl-4 py-2">
                    ...
                    {review.title && (
                      <h5 className="font-semibold text-primary2 mb-1">{review.title}</h5>
                    )}
                    {review.description && (
